        ...
        <el-main style="padding: 0">
            ...
            <el-dialog title="新增供应商" :visible.sync="AddFormVisible">
                <el-form :model="form" :rules="rules" class="demo-ruleForm">
                    <el-form-item label="供应商名" :label-width="formLabelWidth" prop="provName">
...
export default {
    ...
    methods: {
        ...
        //更新
        update() {
            axios.put("/api/provider/update", this.form).then((res) => {
        ...
        remove(row) {
            this.$confirm("此操作将永久删除数据", "提示", {type: "info"}).then(() => {
                axios.delete("/api/provider/delete/" + row.id).then((res) => {
                    if (res.data.code === 200) {
                        this.$message.success("更新成功")
                        this.list()
                    } else if (res.data.code === 403) {
        ...
        //新增
        add() {
            axios.post("/api/provider/add", this.form).then((res) => {
                if (res.data.code === 200) {
